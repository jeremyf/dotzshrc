...
- =brew install emacs-plus@29 --with-debug --with-dbus --with-modern-pen-lds56-icon --with-native-comp --with-mailutils=
...
=ruby install.rb= to create various symlinks and set the defau
